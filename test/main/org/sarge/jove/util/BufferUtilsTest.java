...
public class BufferUtilsTest {
	...
	public void createIntegerBuffer() {
		final IntBuffer buffer = BufferUtils.createIntegerBuffer( 3 );
	...
	public void createIntegerBufferArray() {
		final IntBuffer buffer = BufferUtils.createIntegerBuffer( new int[]{ 1, 2, 3 } );
		assertEquals( ByteOrder.nativeOrder(), buffer.order() );
		check( buffer );
		for( int n = 1; n <= 3; ++n ) {
	...
	public void createFloatBufferArray() {
		final FloatBuffer buffer = BufferUtils.createFloatBuffer( new float[]{ 1, 2, 3 } );
		assertEquals( ByteOrder.nativeOrder(), buffer.order() );
		check( buffer );
		for( int f = 1; f <= 3; ++f ) {
	...
	public void createByteBuffer() {
		final ByteBuffer buffer = BufferUtils.createByteBuffer( 3 );
		assertEquals( ByteOrder.nativeOrder(), buffer.order() );
		check( buffer );
	}
	...
	public void createByteBufferArray() {
		final ByteBuffer buffer = BufferUtils.createByteBuffer( new byte[]{ 1, 2, 3 } );
		assertEquals( ByteOrder.nativeOrder(), buffer.order() );
		check( buffer );
		for( byte b = 1; b <= 3; ++b ) {
