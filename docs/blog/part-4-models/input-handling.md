...
* Allow an application to programatically enumerate the events generated by devices.
...
We next implement an inner template for a GLFW source belonging to this device:
...
Sub-classes of this template are required to implement the following two factory methods to support listener registration:
...
This prevents an 'active' listener from being garbage collected and de-registered by GLFW, i.e. the `listener` argument is out-of-scope at the end of the method.
...
An _axis_ is defined as an event type with a companion class for its associated events:
...
The `listener` factory method creates a GLFW mouse-wheel listener which wraps up the event and delegates it to the handler:
...
Instead we will introduce an _action bindings_ class which is responsible for mapping events to handlers (or methods) and encapsulates switching logic and type casting.
...
We can now bind the ESCAPE key to stop the application:
...
And bind the mouse wheel to a simple handler method:
