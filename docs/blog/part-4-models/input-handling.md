...
* Allow an application to enumerate the types of events generated by each device.
...
We next implement a inner template for a GLFW source belonging to this device:
...
Sub-classes of this template are required to implement the following two methods to support listener registration:

The `listener` method creates a GLFW listener responsible for created an event and delegating to the given handler:
...
And `method` is a provider for the GLFW callback registration method for that listener (similar to how we return a destructor for a Vulkan object):
...
* The key table is hidden but we provide the a factory method to lookup a keyboard key by name.
...
private void handleMouseWheel(float value) { ... }

// Bind mouse-wheel handler
Consumer<Event> handler = e -> {
    AxisEvent axis = (AxisEvent) e;
    handleMouseWheel(axis.value());
};
wheel.bind(handler);
```

However there are still several problems with this approach:

* We are required to cast the event if we want to use the sub-class properties.

* Ideally we would prefer to bind specific _parameterized_ events (especially for the keyboard) rather than having to implement switching logic.

* Additionally we would also like to be able to bind directly to a method reference with an appropriate signature instead of hand-crafting adapters or messy lambdas, e.g. `handleMouseWheel` in the above example.
...
bindings.bind(keyboard.key("ESCAPE"), app::stop);

// Bind mouse-wheel handler
bindings.bind(window.mouse().wheel(), this::handleMouseWheel);
...
The algorithm to calculate the position of the camera is the same as the default controller except for the final step, we therefore refactor the base-class by moving that step to a local helper which can be overridden:
