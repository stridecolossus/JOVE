...
This requires the library to be available on the Java library path, which be configured via the `` environment variable, or by setting the path explicitly in the command line by adding:

`-Djava.library.path=C:\Program Files\RenderDoc`

Note this command line argument this will be need to be in quotes on the Windows platform.
