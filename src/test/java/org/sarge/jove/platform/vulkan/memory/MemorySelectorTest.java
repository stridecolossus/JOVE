...
class MemorySelectorTest {
	...
	@BeforeEach
	void before() {
		properties = new MemoryProperties.Builder<>().usage(VkImageUsageFlag.SAMPLED);
		type = new MemoryType(0, new Heap(42, Set.of()), Set.of(VkMemoryProperty.DEVICE_LOCAL));
	...
	@Test
	void optimal() {
		properties.optimal(VkMemoryProperty.DEVICE_LOCAL);
	...
	void fallback() {
		properties.required(VkMemoryProperty.DEVICE_LOCAL);
		properties.optimal(VkMemoryProperty.PROTECTED);
		assertEquals(Optional.of(type), selector.select(0x1, properties.build()));
	}
	...
	@Test
	void none() {
		properties.required(VkMemoryProperty.HOST_COHERENT);
