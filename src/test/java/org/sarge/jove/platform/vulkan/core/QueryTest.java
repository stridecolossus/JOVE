...
public class QueryTest {
	...
	class DefaultQueryTests {
		...
		void begin() {
			...
			verify(lib).vkCmdBeginQuery(buffer, pool, 0, EnumMask.of(VkQueryControlFlag.PRECISE));
		}
	...
	class PoolTest {
		...
		void create() {
			...
			expected.pipelineStatistics = EnumMask.of();
	...
	void statistics() {
		...
		expected.queryCount = 1;
		expected.pipelineStatistics = EnumMask.of(VkQueryPipelineStatisticFlag.VERTEX_SHADER_INVOCATIONS);
		verify(lib).vkCreateQueryPool(dev, expected, null, dev.factory().pointer());
	}
	...
	class ResultBuilderTest {
		...
		class AccessorTests {
			...
			void accept() {
				accessor.accept(bb);
				verify(lib).vkGetQueryPoolResults(dev, pool, 0, 2, bb.remaining(), bb, 4, EnumMask.of(WAIT));
			}
			...
			void acceptLongValues() {
				...
				verify(lib).vkGetQueryPoolResults(dev, pool, 0, 2, bb.remaining(), bb, 8, EnumMask.of(WAIT, VkQueryResultFlag.LONG));
			}
		...
		void copy() {
			...
			verify(lib).vkCmdCopyQueryPoolResults(buffer, pool, 0, 2, dest, 0, 4, EnumMask.of());
		}
	}
}
