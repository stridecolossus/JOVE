...
class FrameCounterTest {
	...
	void update() {
		...
		counter.frame(new Frame(start, start.plusMillis(50)));
	...
	void cull() {
		...
		counter.frame(new Frame(one, one.plusMillis(50)));

		// Record new frame
		final Instant two = Instant.ofEpochMilli(1000);
		counter.frame(new Frame(two, two.plusMillis(50)));
	...
	void count() {
		...
		for(int n = 0; n < 60; ++n) {
			...
			counter.frame(new Frame(start, start.plusMillis(elapsed)));
		}
