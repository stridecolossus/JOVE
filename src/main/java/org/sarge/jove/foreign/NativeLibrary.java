...
/**
 * A <i>native library</i> is a proxy implementation of a native API that delegates method calls to the underlying FFM implementation.
 * <p>
...
public class NativeLibrary {
	 ...
	 */
	private NativeLibrary(Class<?>[] api, Map<Method, NativeMethod> methods) {
		final ClassLoader loader = this.getClass().getClassLoader();
    	this.proxy = Proxy.newProxyInstance(loader, api, handler(methods));
	}

	/**
	 * @return Invocation handler that delegates API calls to the underlying native method
	 */
	...
	/**
	 * Maps this library to a given API.
	 * @param <T> API
	 * @return API
	 */
	...
	public static class Builder {
    	 ...
    	 */
    	public Builder(SymbolLookup lookup, Registry registry) {
    	 ...
    	 */
    	public NativeLibrary build(List<Class<?>> api) {
    		// Enumerate API methods
    		final Map<Method, NativeMethod> methods = api
        			.stream()
        			.peek(Builder::validate)
        	...
        	return new NativeLibrary(interfaces, methods);
    	}
    	...
    	private NativeParameter parameter(Parameter parameter) {
    		...
    		return new NativeParameter(transformer, isReturnedParameter(parameter));
    	}

    	private static boolean isReturnedParameter(Parameter parameter) {
    		return NativeReference.class.isAssignableFrom(parameter.getType()) || parameter.isAnnotationPresent(Returned.class);
    	}
    }
}
