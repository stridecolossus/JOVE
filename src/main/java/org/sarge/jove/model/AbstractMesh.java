...
//	private final MeshBuilder builder;
	 ...
	 */
	protected AbstractMesh( BufferedMesh mesh ) {
...
//		if( !builder.isReady() ) throw new IllegalArgumentException( "Mesh buffers have not been built" );
		...
		// Create VBOs
		final MeshLayout layout = mesh.getLayout();
		for( int n = 0; n < layout.getBufferLayout().size(); ++n ) {
			...
			for( int c = 0; c < b.getBufferDataTypes().size(); ++c ) {
				...
				verify();
				offset += size * BufferDataType.FLOAT_SIZE; // TODO - assumes float
			}
		}
		...
		final IntBuffer indices = mesh.getIndexBuffer();
		if( indices == null ) {
			indexBuffer = null;
