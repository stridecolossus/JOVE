...
public class MeshBuilder {
	...
	public void computeNormals( int start, int end ) {
		...
		// Working data
		final MutableVector u = new MutableVector();
		final MutableVector v = new MutableVector();
		boolean even = true;
		...
		while( itr.hasNext() ) {
			...
			for( int c = 0; c < 3; ++c ) {
				...
				calcVector( tri, c + 1, pt, u );
				calcVector( tri, c + 2, pt, v );
	 ...
	 */
	private void calcVector( Triangle tri, int idx, Point start, MutableVector vec ) {
		final Point end = tri.getVertex( idx % 3 ).getPosition();
		vec.subtract( end, start ); // TODO - this looks dodgy
	}
