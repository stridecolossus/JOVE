...
public class Scene {
	...
	private final Comparator<Node> DISTANCE_ORDER = new Comparator<Node>() {
		...
		private float distance( Node n ) {
			...
			final Point pos = n.getWorldTransform().toMatrix().multiply( centre );
	...
	public List<Node> pick( Location loc ) {
		...
		final Vector vec = matrix.multiply( Vector.Z_AXIS.invert() );
		final Ray ray = new Ray( new Point( x, y, 0 ), vec );
