...
public abstract class AbstractDemo implements Application {
	...
	private final ImageLoader imageLoader = new ImageLoader( src );
	private final ActionBindings bindings = new ActionBindings();
	private final RenderThreadQueue queue = new RenderThreadQueue();
	...
	@Override
	public final void init( int width, int height, RenderingSystem sys ) throws Exception {
		// Create scene
		final Scene scene = new Scene( sys.createViewport(), new Rectangle( 0, 0, width, height ), new PerspectiveProjection() );
		scenes.add( scene );
